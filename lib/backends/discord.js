...
class Discord extends AbstractBackend {
  ...
  writeMessage(message) {
    ...
    if (message.addEmbed) {
      ...
      if (message.color) {
        try {
          embed.color = parseInt(message.color, 16);
        } catch (e) {
          logger.error("[Discord] Could not parse color: " + e);
        }
      }
      if (message.fields) {
      ...
      if (message.author) {
        if (typeof message.author === "string") {
          embed.author = { name: message.author };
        } else {
          embed.author = message.author;
        }
      }
      if (message.thumbnail) {
        embed.thumbnail = { url: message.thumbnail };
      }
