...
class MessagesHandler {
  ...
  /**
   * Converts a string message to a RichMessage instance if needed
   * @private
   * @param {string|RichMessage} message
   * @returns {RichMessage} A new RichMessage instance if conversion was needed, same value as the input otherwise
   */
  convertMessageIfNeeded(message) {
  ...
  registerBackend(backend) {
    if (backend === undefined || backend === null) {
      throw new Error(
        "MessagesHandler - registerBackend: 'backend' must be a instance of AbstractBackend"
      );
    }
  ...
  registerPlugin(plugin) {
    if (plugin === undefined || plugin === null) {
      throw new Error(
        "MessagesHandler - registerPlugin: 'plugin' must be a instance of AbstractPlugin"
      );
    }
  ...
  messageReceived(backend, nickname, message) {
    if (backend === undefined || backend === null) {
      throw new TypeError(
        "MessagesHandler - messageReceived: backend must be a AbstractBackend instance"
      );
    }
    if (nickname === undefined || nickname === null) {
      throw new TypeError(
        "MessagesHandler - messageReceived: nickname must be a string"
      );
    }
    if (message === undefined || message === null) {
      throw new TypeError(
        "MessagesHandler - messageReceived: message must be a string"
      );
    }

    logger.debug(
      "[MessagesHandler] - [" + backend.name + "] <" + nickname + "> " + message
    );
    ...
    this.plugins
      .filter(plugin => {
        if (plugin.isExcludedFromBackend(backendName)) {
          logger.debug(
            "[MessagesHandler] - Plugin '" +
              plugin.name +
              "' is excluded for the backend '" +
              backendName +
              ". Not delivering."
          );
          return false;
   ...
   * @param {string|RichMessage} message Message to send
   */
  sendMessage(backends, message) {
    message = this.convertMessageIfNeeded(message);
    ...
    logger.debug("Sending to backend " + backends + ": " + message.basicText);
  ...
  sendMessageExcluding(excludedBackends, message) {
    ...
    logger.debug(
      "Sending to all backends except " +
        excludedBackends +
        ": " +
        message.basicText
    );

    outer: for (let backend of this.backends) {
