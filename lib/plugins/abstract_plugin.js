...
class AbstractPlugin {
  ...
  constructor(options) {
    ...
    if (!(this._excludedBackends instanceof Array)) {
      throw new TypeError(
        "AbstractPlugin (" +
          this.name +
          ") - constructor: excluded_backends should be null or a string array"
      );
    }
  }
  ...
  isExcludedFromBackend(backend) {
    if (typeof backend !== "string") {
      throw new TypeError(
        "AbstractPlugin - isExcludedFromBackend: backend should be a string"
      );
    }
  ...
  get name() {
    throw new Error(
      "AbstractPlugin - 'get name' not implemented or called the base implementation"
    );
  }
  ...
  onMesssage(backend, nickname, message) {
    throw new Error(
      "'onMesssage' not implemented or called the base implementation"
    );
  }
}
