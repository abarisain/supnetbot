...
class Coin {
  ...
  fetch() {
    Promise.all(this.fetchUsd(), this.fetchEur()).then(res => {
      return this;
    });
  }
  ...
  toBasicString() {
    return `${this.name.toUpperCase()}: $ ${usdValue} - ${eurValue} €`;
  }
}
  ...
  onCommand(backend, args) {
    ...
    Promise.all([
      ...
      .then(res => {
        ...
        message.title = "GDAX Tickers";
        message.fields = res.map(c => c.toRichMessageField());
